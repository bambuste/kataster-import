...
def main():
    ...
    podporovane_objekty = []
    for vrstvy in list(objects_selection.values()):
    ...
    try:
        ...
        for riadok in vstup:
            ...
            if typ in ('&V', '&R', '&B'):
                ...
                if typ == '&B':
                    ...
                    if atributy.get('TYP') not in list(objects_selection.keys()):
        ...
        for raw_objekt in raw_objekty:
                ...
                for riadok in raw_objekt["riadky"]:
                    try:
                        ...
                        if isinstance(objekt, data.INE_LINIE):
                            logging.warning("Nespracovany riadok: %s", riadok)
                        else:
                            objekt = data.INE_LINIE(atributy, nazov_vrstvy)
                            for riadok in raw_objekt["riadky"]:
                                try:
            ...
            if objekt_data.get('geometricke_objekty'):
                ...
                logging.warning("Objekt %s neobsahuje platnu geometriu, vynechavam jeho ulozenie" % objekt_id)
